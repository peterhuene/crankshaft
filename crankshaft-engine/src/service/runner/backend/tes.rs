            ...
            if let Some(ref state) = task.state {
                match state {
                    State::Unknown | State::Queued | State::Initializing => {
                        ...
                        trace!("task is not yet running; waiting before polling again");
                    }
                    State::Running | State::Paused => {
                        trace!("task is running; waiting before polling again");

                        // Task is running (or was previously running but now paused), so notify
                        if let Some(started) = started.take() {
                            debug!("task `{task_id}` has started");
                            started.send(()).ok();
                        }
                    }
                    State::Complete | State::ExecutorError | State::SystemError => {
                        // Repeat with a full request
                        let task = client
                        ...
                        if *state == State::Complete {
                            debug!("task `{task_id}` has completed");
                        } else {
                            debug!("task `{task_id}` has failed");
                        }
    ...
    fn run(
        ...
        Ok(async move {
            ...
            select! {
                ...
                res = Self::wait_task(&client, &task_id, poll_interval, started) => {
                    res
                }
            }
        }
